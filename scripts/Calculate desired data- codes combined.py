# This file contains codes to calculate aaindex1 data (also given in '5- reformat dataset for ML training.ipynb' file under the Jupyter Notebooks directory).
# The Python codes required to calculate detected and undected peptides, SpC - Spectral counts are given in the respective ipynb file under the Jupyter Notebooks directory.
...
def calculate_aaindex1_features(peptide_df_file, aaindex1_df_file, output_file_name):
    ...
    peptide_df.iloc[0:, 5:] = all_peptide_vals  # column index to add aaindex1 values will vary by df, need to specify
